...
function inicializar(par) {
  if (par==1) {
	...
	document.formimpresion.propiedadesreport1.value = opener.document.descarga.propiedadesreport1.value;
  	//alert(document.formimpresion.sql.value);
	buscarPapel();
  }
}
...
<?
	if( $_POST["archivo2"] == "../tramites/remito.xml"
		|| $_POST["archivo2"] == "../tramites/remitodes.xml" ) {
		/********************************* Caratula  ********************************************/
		$prueba = "SELECT 1";
	...
	/////////////////
	if(!isset($_GET['desde'])) $_GET['desde']=1;
	if(!isset($_GET['hasta'])) $_GET['hasta']=1;
	for( $rm = $_GET['desde'] ; $rm <= $_GET['hasta'] ; $rm++ ) {
