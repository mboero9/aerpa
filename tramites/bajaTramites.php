...
//alert(results);
			...
			/*Modificado para que muestre el tipo de remito (origen o destino)
			else if (results[0]=='Con remito')  {
				alert("Nro. de  tramite:"+buscadox+document.form.NroTramite.value.toUpperCase()+" tiene remito generado.\n");
				inicializar();
			}
			*/
			   ...
			   if (results.length<=2) {
			    ...
			    for (i=0;i<(results.length-1);i++) {
						...
						document.getElementById(f).innerHTML=results2[9];												
...
function ir(i) {
	if (document.formsubmit.opcion.value==0) { //modificacion
	    document.formsubmit.action='bajaTramitesBack.php';
	}
...
<tr><td class=celdatitulo align=center colspan=5>Seleccione el Tr&aacute;mite a Eliminar.</td></tr>
<?
$clase='fondotabla1';
for ($i=0;$i<=40;$i++) { ?>
	<tr><td>
...
<? if ($i==0) { ?>
	<tr><td class=celdatitulo align="center" width="25%">Registro Origen</td>
		<td class=celdatitulo align="center" width="25%">Registro Destino</td>
		<td class=celdatitulo align="center" width="10%">Nro.de Tr&aacute;mite</td>
		<td class=celdatitulo align="center" width="15%">Nro.de Voucher</td>
...
<? } ?>
	<tr class=<?=$clase;?> onClick="seleccionado(<?=$i;?>);" onMouseOver="this.className = 'fondoconfirmacion';" onMouseOut="this.className = '<?=$clase;?>';" style="cursor:pointer">
	    <td class="celdatexto" width="25%"><span id=tramOrig<?=$i;?>></span></td>
		<td class="celdatexto" width="25%"><span id=tramDest<?=$i;?>></span></td>
		<td class="celdatexto" width="10%" align="center"><span id=tramNro<?=$i;?>></span></td>
		<td class="celdatexto" width="15%" align="center"><span id=nroVoucher<?=$i;?>></span></td>
		<td class="celdatexto" width="20%" align="center"><span id=tramFec<?=$i;?>></span>
		<input type=hidden id=tramId<?=$i;?> /></td>		
