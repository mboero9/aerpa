...
//alert("valor:"+valor+"tipo:"+tipo);
	switch(tipo) {
	  case 'n':
		...
		break;
	  case 't':
		var validos = /^[a-zA-Z·¡È…ÌÕÛ”˙⁄¸‹Ò—][a-zA-Z·¡È…ÌÕÛ”˙⁄¸‹Ò— \/\.\,\_\-]*$/;
		break;
	  case 'x':
		var validos = /[DTCdtc]/;
		break;
	...
	case 'desregistro':
		  var id=((campo=='O') ? document.form.CodRegOrig.value : document.form.CodRegDest.value);
  	      if (!Valido(id,'n')) {
								 ...
								 alert("El CÛdigo Ingresado es Invalido");
								 return false;
							   }
		  ...
		  if (http.readyState == 4) {
			...
			if (campo=='O') {
				if (results[1]=='D') {
					document.getElementById('spanDesRegOrig').innerHTML="El Registro posee solo funciÛn Destino";
					document.form.CodRegOrig.value="";					
					document.form.CodRegOrig.focus();
				}else{
